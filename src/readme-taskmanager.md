...
3) Добавлены тесты - сохранение и удаление истории, промежуточная проврка по последней задаче, по размеру, проверку замененной задачи.

4) Добавил функцию удаления из истории при удалении задачи по ID и при очистке списка.

Но не уверен что это корректный шаг - что бы не делалось с объектами История должна содержать все происходящие изменения, 

Желательно с отсутствием возможности редактирования на любом уровне доступа.

5) Обнаружил ошибку в поисковике по ID, исправил. Код даже сократился на пару строк.

p.s. знаю что можно сгененрировать задачи листом через for и так же их загрузить через for - но в конкретной задаче копипаст сделать проще. 
